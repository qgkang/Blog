<?php
	header("Content-Type:text/html;charset=gb2312");	//设置编码格式
	...
	//判断文章是否为引用
	if(!empty($artarr[0]['artquote']) && !is_null($artarr[0]['artquote'])){
		//得到引用的来源地址
	...
	<table width="450" border="1" cellpadding="0" cellspacing="0" align="center">
		...
		<tr>
			<td colspan="5" height="25" align="center" valign="middle">您可以通过点击下面的地址来引用该文章，也可以直接点击&nbsp;
				[<a onclick="return artquote('<?php echo $_GET['uid']; ?>','<?php echo $_GET['artid']; ?>')" style="color:#FF6600;">引用</a>]&nbsp;来自动添加到您的博客空间<br />
				<input id="copyadd" onclick="copyaddress()" type="text" size="50" value="<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; ?>" readonly="readonly" style="border: 1px #FFFFFF solid;"  />&nbsp;
			</td>
		</tr>
	</table>
	<?php
	...
	<table width="450" border="1" cellpadding="0" cellspacing="0" align="center">
		...
		<?php
		if(isset($_SESSION['name']) and isset($_GET['uid'])){
		?>
		<tr>
			<td width="100">文章类别：<?php echo $artarr[0]['typename']; ?></td>
			<td width="100" height="25">点击：<?php echo $artarr[0]['hitnum']; ?>&nbsp;次</td>
	...
	<?php
		...
		if(isset($_GET['curpage']) and isset($_GET['artid']) and isset($_SESSION['name']) and isset($_GET['uid'])){
	...
	<table width="450" border="1" cellpadding="0" cellspacing="0" align="center">
    	<tr>
        	<td height="25" align="center" valign="middle" colspan="3">留言列表</td>
        </tr>
     <?php
	 	foreach($reviewarr as $value){
	 ?>
        <tr>
        	<td width="118" height="25" align="left" valign="middle">&nbsp;评论人：<?php echo $value['man']; ?></td>
            <td width="219" align="left" valign="middle">评论时间：<?php echo $value['firsttime']; ?>&nbsp;</td>
			<td width="105" align="center" valign="middle">&nbsp;<?php echo $_SESSION['name'] == $_GET['uid']?'<a onclick="delonereview(\''.$value['id'].'\')">删除</a>':''; ?></td>
		</tr>
		<tr>
      		<td height="50" colspan="3" align="left" valign="middle"><div style="margin:12px;"><?php echo $value['content']; ?></div></td>
        </tr>

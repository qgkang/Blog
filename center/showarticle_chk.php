<?php
	...
	if(empty($act)){
	  if($name != $uid){
		...
		$content = $_GET['cont'];
		$sql = "insert into tb_review (artid,content,man) values('".$artid."','".$content."','".($name == ''?'ÄäÃû':$name)."')";
		...
		if(!empty($name) and !is_null($name)){
			if($_GET['uid'] == $name){
				...
				if($chkquote >= 1){
					...
					$sql = "insert into tb_article (typename,author,artquote) values('ÈÕ¼Ç','".$name."','".$saveid."')";
					$num = $conne->uidRst($sql);
					if($num == 1){
						$reback = '1';
					}else{

<?php
	...
	if(!empty($_GET['uid']) and !is_null($_GET['uid']) and isset($_GET['curpage'])){
		...
		if(empty($curpage) or is_null($curpage)){
			...
			if($arr != ''){ 
?>
<div style="background-image:url(images/header10.gif); background-position:left; background-repeat: no-repeat; color:#f79100; line-height:25px; height:25px; text-indent: 12px; text-align:left; border-bottom: 1px #CCCCCC dashed; ">我的好友</div>
<p />&nbsp;&nbsp;
    ...
    <td align="center" valign="middle">
    	<table border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
            	...
            	<td height="25" align="left" valign="middle"><a href='<?php echo 'center.php?uid='.$showarr[0]['name']; ?>' target="_blank">进入博客</a>&nbsp;</td>
            </tr>
      </table>
    </td>
<?PHP
...
<?php
	?>
		<tr>
			<td colspan="4" height="30" align="right" valign="middle">&nbsp;&nbsp;&nbsp;<?php echo ($curpage == 1)?"首页":"<a  onclick='return artpagination(\"frd.php?act=&uid=".$_GET['uid']."&curpage=1\")'>首页</a>"; ?>&nbsp;<?php echo ($curpage==1)?"上一页":"<a onclick='return artpagination(\"frd.php?act=&uid=".$_GET['uid']."&curpage=".($curpage-1)."\")'>上一页</a>"; ?>&nbsp;<?php echo ($curpage == $totpage)?'下一页':"<a onclick='return artpagination(\"frd.php?act=&uid=".$_GET['uid']."&curpage=".($curpage+1)."\")'>下一页</a>"; ?> <?php echo ($curpage==$totpage)?"尾页":"<a onclick='return artpagination(\"frd.php?act=&uid=".$_GET['uid']."&curpage=".$totpage."\")'>尾页</a>"; ?> 当前是第<?php echo $curpage; ?>页/共<?php echo $totpage; ?>页<?php echo $totnum; ?>条记录  跳转到：

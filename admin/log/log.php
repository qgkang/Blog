...
<?php
	...
	if(empty($_REQUEST['act'])){	//获取当前是第几页
		$act = "";
	}else{
		$act = $_REQUEST['act'];				//当前页	
	}
	
	if(!isset($_REQUEST['curpage'])){	//获取当前是第几页
		$curpage = 1;
	}else{
		$curpage = $_REQUEST['curpage'];				//当前页	
	}
...
?>
	<tr class="top">
		<td colspan="3" height="30"><a href="#" onclick="return alldel(showmem);">全选</a> <a href="#" onclick="return overdel(showmem);">反选</a>&nbsp;&nbsp;
		  ...
		  <td colspan="7" align="right" valign="middle"><?php echo ($curpage == 1)?"首页":"<a href=log.php?curpage=1&querymem=".$querymem."&signslt=".urlencode($signslt)."&cont=".$cont.">首页</a>"; ?>&nbsp;<?php echo ($curpage==1)?"上一页":"<a href=log.php?curpage=".($curpage-1)."&querymem=".$querymem."&signslt=".urlencode($signslt)."&cont=".$cont.">上一页</a>"; ?>&nbsp;<?php echo ($curpage == $totpage)?'下一页':"<a href=log.php?curpage=".($curpage+1)."&querymem=".$querymem."&signslt=".urlencode($signslt)."&cont=".$cont.">下一页</a>"; ?> <?php echo ($curpage==$totpage)?"尾页":"<a href=log.php?curpage=".$totpage."&querymem=".$querymem."&signslt=".urlencode($signslt)."&cont=".$cont.">尾页</a>"; ?> 当前是第<?php echo $curpage; ?>页/共<?php echo $totpage; ?>页<?php echo $totnum; ?>条记录  跳转到：<select id="jump" name="jump" onchange="return jumpmem('log.php','<?php echo $sql; ?>')">
		  	<?php
				for($i=1;$i<=$totpage;$i++){
					if($i == $curpage)
